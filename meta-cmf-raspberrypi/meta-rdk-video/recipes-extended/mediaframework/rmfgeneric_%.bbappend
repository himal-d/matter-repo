...
SRC_URI_append = " \
    ...
    file://0001_remove_check_rf_presence.patch;apply=no \
"
addtask rmf_patches after do_unpack do_patch before do_configure

do_rmf_patches() {
    cd ${S}
    if [ ! -e patch_applied ]; then
        bbnote "Patching 0001_remove_check_rf_presence.patch from '${WORKDIR}'"
        patch -p1 < ${WORKDIR}/0001_remove_check_rf_presence.patch
        touch patch_applied
    fi
}
