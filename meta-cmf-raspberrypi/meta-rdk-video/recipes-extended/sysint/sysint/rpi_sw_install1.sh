...
if [ "$(ls -A $pwd)" ]; then
  ...
  devcs=`sha256sum /extblock/tftpimage/imagedwnld/rdk* | cut -f 1 -d " "`       
...
httpDownload () {
    ...
    cd checksum
    check_sum=$(echo "$cloudFWFile")
    check_sum_file="${check_sum}_hmac.txt"
    ...
    if [ "$(ls -A $pwd)" ]; then
          ...
          cloudcs=`cat $cloudcsfile_path | cut -f 1 -d ' '`
          echo "cloudcs:cloud download md5sum file version is:$cloudcs"
          devcs=`sha256sum /extblock/httpimage/imagedwnld/$cloudFWFile | cut -f 1 -d " "`
          echo "devcs:image download checksum md5sum file version is:$devcs"
          if [ "$devcs" = "$cloudcs" ]; then
             echo "md5sum matches !!"
             ret=0
...
fdisk  -l $file  > /extblock/sector.txt
else
fdisk -u -l $file  > /extblock/sector.txt
fi

if [ `cat /extblock/sector.txt | grep -c "(non-Linux?):"` != "0" ];then

 `sed '/Partition\|phys/d' /extblock/sector.txt >   /extblock/sector1.txt`
fi

if [ `fdisk -l | grep -c "CHS"` == "1" ] && [ `cat /extblock/sector.txt | grep -c "(non-Linux?):"` != "0" ];then

linux_sector=`tail -2 /extblock/sector1.txt | tr -s ' ' | cut -d'*' -f2 | cut -d' ' -f4 | head -n1`
linux_offset=$((linux_sector*512))

rootfs_sector=`tail -2 /extblock/sector1.txt | tr -s ' ' | cut -d'*' -f2 | cut -d' ' -f4 | tail -1`
rootfs_offset=$((rootfs_sector*512))

elif [ `fdisk -l | grep -c "CHS"`   == "1" ] &&  [ `cat /extblock/sector.txt | grep -c "(non-Linux?):"` == "0" ]; then

linux_sector=`tail -2 /extblock/sector.txt | tr -s ' ' | cut -d'*' -f2 | cut -d' ' -f4 | head -n1`
linux_offset=$((linux_sector*512))

rootfs_sector=`tail -2 /extblock/sector.txt | tr -s ' ' | cut -d'*' -f2 | cut -d' ' -f4 | tail -1`
rootfs_offset=$((rootfs_sector*512))

elif [ `fdisk -l | grep -c "CHS"` == "0" ] && [ `cat /extblock/sector.txt | grep -c "(non-Linux?):"` != "0" ]; then
       
linux_sector=`tail -2 /extblock/sector1.txt | tr -s ' ' | cut -d'*' -f2 | cut -d' ' -f2 | head -n1`
linux_offset=$((linux_sector*512))

rootfs_sector=`tail -2 /extblock/sector1.txt | tr -s ' ' | cut -d'*' -f2 | cut -d' ' -f2 | tail -1`
rootfs_offset=$((rootfs_sector*512))
...
rm -rf /extblock/bank*
rm -f /extblock/sec*
echo "Deleting downloaded image  from server as the fw upgrade is ok"
if [ $protocol -eq 1 ]; then
