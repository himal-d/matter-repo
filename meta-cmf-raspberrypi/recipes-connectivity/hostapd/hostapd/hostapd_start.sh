...
WLAN24G=`cat /nvram/hostapd0.conf | grep interface= | head -n1 | cut -d '=' -f2`
PWLAN24G=`cat /nvram/hostapd4.conf | grep interface= | head -n1 | cut -d '=' -f2`
WLAN5G=`cat /nvram/hostapd1.conf | grep interface= | head -n1 | cut -d '=' -f2`                                                                                                                 
PWLAN5G=`cat /nvram/hostapd5.conf | grep interface= | head -n1 | cut -d '=' -f2`             
                                                                                                    
PHY_2G=`iw $WLAN24G info | grep phy | cut -d ' ' -f2`                                                                 
PPHY_2G=`iw $PWLAN24G info | grep phy | cut -d ' ' -f2`                                                                 
PHY_5G=`iw $WLAN5G info | grep phy | cut -d ' ' -f2`                                                                  
PPHY_5G=`iw $PWLAN5G info | grep phy | cut -d ' ' -f2`                                                                  
...
hostapd_cli -i global raw ADD bss_config=$PPHY_2G:/nvram/hostapd4.conf                                                 
hostapd_cli -i global raw ADD bss_config=$PPHY_5G:/nvram/hostapd5.conf  
...
{
	hostapd_cli  -i $1 SET ignore_broadcast_ssid 1
	hostapd_cli  -i $1 disable
	hostapd_cli  -i $1 enable
}
...
if [ "$HOTSPOT_ENABLE" -eq 0 ]; then
	...
	Disabling_XfinityWiFi $wifi_oldinterface 
